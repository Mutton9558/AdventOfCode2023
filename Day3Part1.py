...
for rowIndex, row in enumerate(grid):
    for colIndex, symbol in enumerate(row):
        if symbol.isdigit() or symbol == ".":
            continue
        else:
            for currentRow in [rowIndex-1, rowIndex, rowIndex+1]:
                for currentCol in [colIndex-1, colIndex, colIndex+1]:
                    if currentRow < 0 or currentRow>=len(grid) or currentCol < 0 or currentCol >= len(grid[currentRow]) or not grid[currentRow][currentCol].isdigit():
                        continue
                    while currentCol > 0 and grid[currentRow][currentCol-1].isdigit():
                        currentCol -= 1
                    coordSet.add((currentRow,currentCol))
numberSet = []
for r, c in coordSet:
    s = ""
